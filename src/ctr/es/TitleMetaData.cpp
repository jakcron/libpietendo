#include <pietendo/ctr/es/TitleMetaData.h>
#include <fmt/core.h>
...
pie::ctr::es::TitleMetaDataDeserialiser::TitleMetaDataDeserialiser(const std::shared_ptr<tc::io::IStream>& tmd_stream) :
	TitleMetaData(),
	mModuleLabel("pie::ctr::es::TitleMetaDataDeserialiser")
{
	...
	tc::ByteData tmd_data = tc::ByteData(sizeof(pie::es::ESV1TitleMeta));
	...
	*/
	...
	if (tmd->head.type.unwrap() != pie::es::ESTitleType_CT_TITLE)
	{
	...
	*/
	...
	for (size_t i = 0; i < cmd_table_num; i++)
	{
		this->content_info.push_back(ContentInfo(
			...
			(tmd->contents[i].type.unwrap() & (uint16_t)pie::es::ESContentType_ENCRYPTED) == (uint16_t)pie::es::ESContentType_ENCRYPTED,
			(tmd->contents[i].type.unwrap() & (uint16_t)pie::es::ESContentType_OPTIONAL) == (uint16_t)pie::es::ESContentType_OPTIONAL,
