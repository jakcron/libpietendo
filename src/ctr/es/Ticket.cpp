#include <pietendo/ctr/es/Ticket.h>
#include <fmt/core.h>
...
pie::ctr::es::TicketDeserialiser::TicketDeserialiser(const std::shared_ptr<tc::io::IStream>& tik_stream) :
	Ticket(),
	mModuleLabel("pie::ctr::es::TicketDeserialiser")
{
	...
	tc::ByteData tik_data = tc::ByteData(sizeof(pie::es::ESV1Ticket));
	...
	// update pointer
	tik = (pie::es::ESV1Ticket*)tik_data.data();
	...
	//pie::es::ESV1SectionHeader* sect_hdr = (pie::es::ESV1SectionHeader*)(tik_data.data() + sizeof(pie::es::ESTicket) + tik->v1Head.sectHdrOfst.unwrap());
	...
	*/
	...
	for (size_t i = 0; i < tik->head.limits.size(); i++)
	{
		if (tik->head.limits[i].code.unwrap() == (uint32_t)pie::es::ESLimitCode_NUM_LAUNCH)
		{
	...
	{
		if (tik->sectHdrs[i].sectionType.unwrap() == (uint32_t)pie::es::ESItemType_CONTENT)
		{
			if (tik->sectHdrs[i].recordSize.unwrap() != sizeof(pie::es::ESV1ContentRecord))
			{
				throw tc::InvalidOperationException(mModuleLabel, fmt::format("Invalid size for ESV1ContentRecord. (expected: 0x{:x}, got 0x{:x})", sizeof(pie::es::ESV1ContentRecord), tik->sectHdrs[i].recordSize.unwrap()));
			}
			pie::es::ESV1ContentRecord* content_records = (pie::es::ESV1ContentRecord*)(tik_data.data() + sizeof(pie::es::ESTicket) + tik->sectHdrs[i].sectOfst.unwrap());
