...
pie::ctr::ExeFsSnapshotGenerator::ExeFsSnapshotGenerator(const std::shared_ptr<tc::io::IStream>& stream, bool verify_hashes) :
	FileSystemSnapshot()
{
	// validate stream properties
	if (stream == nullptr)
	{
		throw tc::ObjectDisposedException("pie::ctr::ExeFsSnapshotGenerator", "Failed to open input stream.");
	}
	if (stream->canRead() == false || stream->canSeek() == false)
	{
		throw tc::NotSupportedException("pie::ctr::ExeFsSnapshotGenerator", "Input stream requires read/seek permissions.");
	}
	...
	int64_t pos = 0;
	for (size_t i = 0; i < pie::ctr::ExeFsHeader::kFileNum; i++)
	{
		// skip empty entry
		...
		{
			throw tc::ArgumentOutOfRangeException("pie::ctr::ExeFsSnapshotGenerator", "ExeFs section had unexpected offset.");
		}
		...
		tmp.hash = hdr.hash_table[pie::ctr::ExeFsHeader::kFileNum - 1 - i];

		pos = align<int64_t>((static_cast<int64_t>(tmp.offset) + static_cast<int64_t>(tmp.size)), pie::ctr::ExeFsHeader::kExeFsSectionAlignSize);
	...
	for (size_t i = 0; i < section.size(); i++)
	{
		...
		{			
			...
			{
				tmp.stream = std::make_shared<tc::io::SubStream>(tc::io::SubStream(stream, static_cast<int64_t>(section[i].offset) + static_cast<int64_t>(sizeof(pie::ctr::ExeFsHeader)), static_cast<int64_t>(section[i].size)));
			}
