...
void pie::ctr::CtrKeyGenerator::GenerateKey(const uint8_t *x, const uint8_t *y, uint8_t *key)
{
...
int32_t pie::ctr::CtrKeyGenerator::wrap_index(int32_t i)
{
	return i < 0 ? ((i % 16) + 16) % 16 : (i > 15 ? i % 16 : i);
}

void pie::ctr::CtrKeyGenerator::n128_rrot(const uint8_t *in, uint32_t rot, uint8_t *out)
{
...
void pie::ctr::CtrKeyGenerator::n128_lrot(const uint8_t *in, uint32_t rot, uint8_t *out)
{
	uint32_t bit_shift, byte_shift;
...
/* out = a + b
*/
void pie::ctr::CtrKeyGenerator::n128_add(const uint8_t *a, const uint8_t *b, uint8_t *out)
{
	uint8_t carry = 0;
	uint32_t sum = 0;
...
/* out = a - b
*/
void pie::ctr::CtrKeyGenerator::n128_sub(const uint8_t *a, const uint8_t *b, uint8_t *out)
{
	uint8_t carry = 0;
	uint32_t sum = 0;
...
void pie::ctr::CtrKeyGenerator::n128_xor(const uint8_t *a, const uint8_t *b, uint8_t *out)
{
	for (int i = 0; i < 16; i++) {
		out[i] = a[i] ^ b[i];
	}
