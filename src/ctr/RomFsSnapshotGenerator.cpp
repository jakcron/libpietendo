...
pie::ctr::RomFsSnapshotGenerator::RomFsSnapshotGenerator(const std::shared_ptr<tc::io::IStream>& stream) :
	FileSystemSnapshot(),
	mBaseStream(stream),
	mDataOffset(0),
	mDirEntryTable(),
	...
	mFileEntryTable()
{
	...
	{
		throw tc::ObjectDisposedException("pie::ctr::RomFsSnapshotGenerator", "Failed to open input stream.");
	}
	if (mBaseStream->canRead() == false || mBaseStream->canSeek() == false)
	{
		throw tc::NotSupportedException("pie::ctr::RomFsSnapshotGenerator", "Input stream requires read/seek permissions.");
	}
	...
	mBaseStream->read((byte_t*)(&hdr), sizeof(pie::ctr::RomFsHeader));

	/*
	std::cout << "hdr.header_size             : " << hdr.header_size.unwrap() << std::endl;
	std::cout << "sizeof(pie::ctr::RomFsHeader)  : " << sizeof(pie::ctr::RomFsHeader) << std::endl;
	...
	std::cout << "expected data offset        : " << align<uint32_t>(hdr.file_entry.offset.unwrap() + hdr.file_entry.size.unwrap(), pie::ctr::RomFsHeader::kRomFsDataAlignSize) << std::endl;
	*/


	if (hdr.header_size.unwrap() != sizeof(pie::ctr::RomFsHeader) ||
	    hdr.dir_hash_bucket.offset.unwrap() != sizeof(pie::ctr::RomFsHeader) ||
	    hdr.data_offset.unwrap() != align<uint32_t>(hdr.file_entry.offset.unwrap() + hdr.file_entry.size.unwrap(), pie::ctr::RomFsHeader::kRomFsDataAlignSize))
	{
		throw tc::ArgumentOutOfRangeException("pie::ctr::RomFsSnapshotGenerator", "RomFsHeader is corrupted.");
	}
	...
	*/
	...
	for (uint32_t v_addr = 0; v_addr < mFileEntryTable.size();)
	{
		...
		uint32_t total_size = sizeof(pie::ctr::RomFsFileEntry) + align<uint32_t>(getFileEntry(v_addr)->name_size.unwrap(), 4);
		std::cout << " > entry_size:   0x" << std::hex << total_size << std::endl;
	...
	*/
	...
	{
		throw tc::ArgumentOutOfRangeException("pie::ctr::RomFsSnapshotGenerator", "Root RomFsDirectoryEntry corrupted.");
	}
	...
	for (uint32_t v_addr = 0; v_addr < mDirEntryTable.size();)
	{
		...
		else
		{
			...
			if (mDirParentVaddrMap.find(getDirEntry(v_addr)->parent_offset.unwrap()) == mDirParentVaddrMap.end())
				throw tc::InvalidOperationException("pie::ctr::RomFsSnapshotGenerator", "Directory had invalid parent");
	...
	for (uint32_t v_addr = 0; v_addr < mFileEntryTable.size();)
	{
		...
		if (mDirParentVaddrMap.find(getFileEntry(v_addr)->parent_offset.unwrap()) == mDirParentVaddrMap.end())
			throw tc::InvalidOperationException("pie::ctr::RomFsSnapshotGenerator", "File had invalid parent");
		...
		if (getFileEntry(v_addr)->sibling_offset.unwrap() < v_addr)
		{
			throw tc::InvalidOperationException("pie::ctr::RomFsSnapshotGenerator", "Possibly corrupted file entry");
		}
...
void pie::ctr::RomFsSnapshotGenerator::addFile(const pie::ctr::RomFsFileEntry* file_entry, size_t parent_dir)
{
	// create file entry
	FileEntry tmp;
...
void pie::ctr::RomFsSnapshotGenerator::addDirectory(const pie::ctr::RomFsDirectoryEntry* dir_entry, size_t parent_dir)
{
	// create dir entry
	DirEntry tmp;
