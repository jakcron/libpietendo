...
pie::ctr::IvfcStream::IvfcStream() :
	mModuleLabel("pie::ctr::IvfcStream"),
	mBaseStream(),
...
pie::ctr::IvfcStream::IvfcStream(const std::shared_ptr<tc::io::IStream>& stream) :
	IvfcStream()
{
	mBaseStream = stream;
	...
	{
		throw tc::ObjectDisposedException("pie::ctr::IvfcStream", "stream is null.");
	}
	if (mBaseStream->canRead() == false)
	{
		throw tc::InvalidOperationException("pie::ctr::IvfcStream", "stream does not support reading.");
	}
	if (mBaseStream->canSeek() == false)
	{
		throw tc::InvalidOperationException("pie::ctr::IvfcStream", "stream does not support seeking.");
	}
	...
	pie::ctr::IvfcCtrRomfsHeader hdr;
	mBaseStream->seek(0, tc::io::SeekOrigin::Begin);
	mBaseStream->read((byte_t*)&hdr, sizeof(pie::ctr::IvfcCtrRomfsHeader));

	if (hdr.head.struct_magic.unwrap() != pie::ctr::IvfcHeader::kStructMagic)
	{
		throw tc::ArgumentOutOfRangeException("pie::ctr::IvfcStream", "IVFC header had invalid struct magic.");
	}
	if (hdr.head.type_id.unwrap() != pie::ctr::IvfcHeader::TypeId_A)
	{
		throw tc::ArgumentOutOfRangeException("pie::ctr::IvfcStream", "IVFC header had unexpected type id.");
	}
	...
	for (size_t i = 0; i < section.size(); i++)
	{
		if (i == MasterHash)
		{
			section[i].offset = int64_t(align<size_t>(sizeof(pie::ctr::IvfcCtrRomfsHeader), pie::ctr::IvfcCtrRomfsHeader::kHeaderAlign));
		...
		else
		{
			...
			{
				throw tc::OutOfMemoryException("pie::ctr::IvfcStream", "IVFC layer offset too large.");
			}
			if (tc::is_uint64_t_too_large_for_int64_t(hdr.level[i-1].size.unwrap()))
			{
				throw tc::OutOfMemoryException("pie::ctr::IvfcStream", "IVFC layer size too large.");
			}
	...
	{
		throw tc::OutOfMemoryException("pie::ctr::IvfcStream", "IVFC master hash table too large.");
	}
	if (tc::is_int64_t_too_large_for_size_t(section[HashLevel0].size))
	{
		throw tc::OutOfMemoryException("pie::ctr::IvfcStream", "IVFC level0 hash table too large.");
	}
	if (tc::is_int64_t_too_large_for_size_t(section[HashLevel1].size))
	{
		throw tc::OutOfMemoryException("pie::ctr::IvfcStream", "IVFC level1 hash table too large.");
	}
	...
	*/
	...
	{
		throw tc::crypto::CryptoException("pie::ctr::IvfcStream", "Hash layer0 failed hash validation.");
	}
...
size_t pie::ctr::IvfcStream::read(byte_t* ptr, size_t count)
{
	...
	*/
	...
	{	
		...
		{
			throw tc::crypto::CryptoException("pie::ctr::IvfcStream", "Data layer block(s) failed hash validation.");
		}
...
void pie::ctr::IvfcStream::setLength(int64_t length)
{
	if (mDataStream == nullptr)
	{
...
void pie::ctr::IvfcStream::dispose()
{
...
bool pie::ctr::IvfcStream::validateLayerBlocksWithHashLayer(const byte_t* layer, size_t block_size, size_t block_num, const byte_t* hash_layer)
{
