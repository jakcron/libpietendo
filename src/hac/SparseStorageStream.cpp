#include <memory>
#include <pietendo/hac/SparseStorageStream.h>
...
pie::hac::SparseStorageStream::SparseStorageStream(const std::shared_ptr<tc::io::IStream>& compacted_stream, int64_t stream_length, const pie::hac::BucketTree& bucket_tree) :
	IndirectStorageStream(compacted_stream, std::make_shared<ZerosStream>(ZerosStream()), stream_length, bucket_tree)
{
}

size_t pie::hac::SparseStorageStream::ZerosStream::read(byte_t* ptr, size_t count)
{
	// track read_count
...
size_t pie::hac::SparseStorageStream::ZerosStream::write(const byte_t* ptr, size_t count)
{
	throw tc::NotImplementedException("ZerosStream::write()", "write is not supported for ZerosStream");
}

int64_t pie::hac::SparseStorageStream::ZerosStream::seek(int64_t offset, tc::io::SeekOrigin origin)
{
